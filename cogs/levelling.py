...
class levelling(commands.Cog, name="levelling"):
    ...
    async def on_message(self, message):
        ...
        cursor.execute('INSERT OR IGNORE INTO message_counts (user_id, message_count) VALUES (?, 0)',
                       (message.author.id,))
        cursor.execute('UPDATE message_counts SET message_count = message_count + 1 WHERE user_id = ?',
                       (message.author.id,))
        conn.commit()
    ...
    async def rank(self, context: Context) -> None:
        ...
        if result:
            ...
            progress_bar = start_emoji + (boxes - 1) * filled_emoji + middle_emoji + unfilled_emoji * (
                        10 - boxes) + end_emoji

            embed = discord.Embed(title="", description=f"# {context.author.name}",
                                  color=discord.Colour(int("2B2D31", 16)))
